    ...
    historico.forEach((item, i) => {
        ...
        if (CONFIG_HISTORICO.incluirDadosAPI && item.dadosAPI) {
            ...
            if (tipo === 'historico') {
                ...
                out += `  Endpoint: ${item.dadosAPI.endpoint || 'N/A'}\n`;
...
function getContextoLeonardo(userId, usinasPermitidas) {
    ...
    const dataMes1Atras = `01/${String(mes1Atras.getMonth() + 1).padStart(2, '0')}/${mes1Atras.getFullYear()} 00:00`;
    const fimMes1Atras = new Date(mes1Atras.getFullYear(), mes1Atras.getMonth() + 1, 0);
    const dataFimMes1Atras = `${fimMes1Atras.toLocaleDateString('pt-BR')} 23:59`;

    const mes2Atras = new Date(agora);
    mes2Atras.setMonth(agora.getMonth() - 2);
    const dataMes2Atras = `01/${String(mes2Atras.getMonth() + 1).padStart(2, '0')}/${mes2Atras.getFullYear()} 00:00`;
    const fimMes2Atras = new Date(mes2Atras.getFullYear(), mes2Atras.getMonth() + 1, 0);
    const dataFimMes2Atras = `${fimMes2Atras.toLocaleDateString('pt-BR')} 23:59`;

    const mes3Atras = new Date(agora);
    mes3Atras.setMonth(agora.getMonth() - 3);
    const dataMes3Atras = `01/${String(mes3Atras.getMonth() + 1).padStart(2, '0')}/${mes3Atras.getFullYear()} 00:00`;
    const fimMes3Atras = new Date(mes3Atras.getFullYear(), mes3Atras.getMonth() + 1, 0);
    const dataFimMes3Atras = `${fimMes3Atras.toLocaleDateString('pt-BR')} 23:59`;

    // Últimos 3 dias
    const tresDiasAtras = new Date(agora);
    tresDiasAtras.setDate(agora.getDate() - 3);
    const dataTresDiasAtras = `${tresDiasAtras.toLocaleDateString('pt-BR')} 00:00`;

    const doisDiasAtras = new Date(agora);
    doisDiasAtras.setDate(agora.getDate() - 2);
    const dataDoisDiasAtras = `${doisDiasAtras.toLocaleDateString('pt-BR')} 00:00`;
    ...
    // Nomenclatura para histórico (lowercase com underscore)
    const nomeUsinaHistorico = nomeUsina.toLowerCase().replace(/-/g, '_');
...
Ou então, para acessar a API de histórico expandida:
{"comando":"historico","parametros":{"endpoint":"producao-acumulada|grupo-usina|sensor-usina|tabela-usina","usina":"NOME_DA_USINA","data_inicio":"DD/MM/YYYY HH:mm","data_fim":"DD/MM/YYYY HH:mm","periodo":"H|D|M","grupo":"nome_grupo","variavel":"nome_variavel"}}
...
Gere o comando JSON para consulta na API de dados históricos expandida. Determine o "endpoint" com base no tipo de query:
- Para energia gerada/produção acumulada: "endpoint": "producao-acumulada", inclua "periodo": "H" (hora), "D" (dia) ou "M" (mês) baseado no intervalo.
- Para grupos de variáveis (ex.: potências, temperaturas, hidráulica): "endpoint": "grupo-usina", inclua "grupo": (ex.: "potencia", "temperaturas", "hidraulica"). Use "hidraulica" para dados de nível de água, vazão e dados hidráulicos.
- Para variável específica (ex.: potência ativa da UG-01, nível montante): "endpoint": "sensor-usina", inclua "variavel": (alias exato, ex.: "UG-01 Potência Ativa").
- Para dump completo de todos os dados: "endpoint": "tabela-usina" (use raramente, só para queries muito amplas como "todos os dados brutos").

Quando não houver referência de tempo e o usuário perguntar sobre valores instantâneos:
...
• Hoje: ${dataAtual} 00:00 até ${dataAtual} 23:59
• Ontem: ${dataOntem} 00:00 até ${dataOntem} 23:59
• Últimos 3 dias: ${dataTresDiasAtras} até ${dataAtual} 23:59
• Últimos 2 dias: ${dataDoisDiasAtras} até ${dataAtual} 23:59
...
- Para datas na nova API histórica, sempre use formato "DD/MM/YYYY HH:mm" com início em 00:00 e fim em 23:59 para dias completos.
- Calcule datas relativas automaticamente (ex.: "últimos 2 dias" usa data_inicio do dia mais antigo 00:00).
- Para usinas com 2 UGs, especifique UG na "variavel" se mencionado (ex.: "UG-01 Potência Ativa"); senão, use grupo inteiro.
- Sempre use nomenclatura UPPERCASE com hífen para "usina" (ex.: "${nomeUsina}").
- Os dados históricos agora incluem múltiplas variáveis; use o endpoint apropriado baseado na query.
...
CONSULTAS HISTÓRICAS (sempre usar "historico"):
    ...
    {"comando":"historico","parametros":{"endpoint":"producao-acumulada","usina":"${nomeUsina}","data_inicio":"${dataMes3Atras}","data_fim":"${dataAtual} 23:59","periodo":"M"}}

    Produção da ${nomeUsina} hoje:
    {"comando":"historico","parametros":{"endpoint":"producao-acumulada","usina":"${nomeUsina}","data_inicio":"${dataAtual} 00:00","data_fim":"${dataAtual} 23:59","periodo":"D"}}

    Dados da ${nomeUsina} últimos 3 dias:
    {"comando":"historico","parametros":{"endpoint":"producao-acumulada","usina":"${nomeUsina}","data_inicio":"${dataTresDiasAtras}","data_fim":"${dataAtual} 23:59","periodo":"D"}}

    Geração mensal da ${nomeUsina} em julho 2025:
    {"comando":"historico","parametros":{"endpoint":"producao-acumulada","usina":"${nomeUsina}","data_inicio":"01/07/2025 00:00","data_fim":"31/07/2025 23:59","periodo":"M"}}

    Potências da ${nomeUsina} ontem:
    {"comando":"historico","parametros":{"endpoint":"grupo-usina","usina":"${nomeUsina}","grupo":"potencia","data_inicio":"${dataOntem} 00:00","data_fim":"${dataOntem} 23:59"}}

    Temperaturas da UG-01 na ${nomeUsina} nos últimos 2 dias:
    {"comando":"historico","parametros":{"endpoint":"grupo-usina","usina":"${nomeUsina}","grupo":"temperaturas","data_inicio":"${dataDoisDiasAtras}","data_fim":"${dataAtual} 23:59"}}

    Nível montante da ${nomeUsina} hoje:
    {"comando":"historico","parametros":{"endpoint":"grupo-usina","usina":"${nomeUsina}","grupo":"hidraulica","data_inicio":"${dataAtual} 00:00","data_fim":"${dataAtual} 23:59"}}

    Nível jusante da ${nomeUsina} ontem:
    {"comando":"historico","parametros":{"endpoint":"grupo-usina","usina":"${nomeUsina}","grupo":"hidraulica","data_inicio":"${dataOntem} 00:00","data_fim":"${dataOntem} 23:59"}}

    Todos os dados brutos da ${nomeUsina} ontem:
    {"comando":"historico","parametros":{"endpoint":"tabela-usina","usina":"${nomeUsina}","data_inicio":"${dataOntem} 00:00","data_fim":"${dataOntem} 23:59"}}

    OBS: Essas respostas vão ser formatadas em código para enviar para o usuário, não sendo retrasmitidas para você.
    Os dados Históricos agora estão disponíveis para múltiplas variáveis, incluindo energia em MWh, potências, temperaturas, níveis de água, etc., via a nova API expandida.
