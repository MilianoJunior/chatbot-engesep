...
class ApiService {
    ...
    async getData(config, data) {
        ...
        // Monta o body com o novo formato de payload
        // registers já está no formato: {"Nome": [endereco, tipo, {opcoes}]}
        const body = {
        ...
        try {
            ...
            fim = (Date.now() - inicio) / 1000; // Converte para segundos
            ...
            fim = (Date.now() - inicio) / 1000; // Calcula tempo mesmo em caso de erro
     ...
     * @param {string} tipo - Tipo de dados ("potencias", "nivel_agua", "gerador", "temperaturas", "monitoramento")
     * @returns {Promise<Array>} [dados, tempo_execucao]
     */
    async getUsinaData(nomeUsina, ug, tipo) {
        ...
        // O novo formato já está correto: {"Nome": [endereco, tipo, {opcoes}]}
        // Apenas pega diretamente do ugData[tipo]
        const registers = ugData[tipo] || {};
        
        // Valida se o formato está correto
        if (typeof registers !== 'object' || registers === null) {
            throw new Error(`Formato inválido para tipo '${tipo}' na UG ${ug}. Esperado um objeto com registradores.`);
        }
